...
namespace blog_api.Helper
{
    public class JwtHelper
    {
        ...
        public class JwtHelpers
        {
            ...
            public string GenerateToken(string userName, string authorization, string device, Dictionary<string, object> claimKeys, int expireMinutes = 60)
            {
                ...
                {
                    claims.Add(new Claim("authorization", authorization, ClaimValueTypes.String, issuer));
                }
                ...
                foreach (var key in claimKeys.Keys)
                {
                    claims.Add(new Claim(key, claimKeys[key].ToString()));
                }
            ...
            /// <returns></returns>
            public string GenerateEncryptionToken(string userName, string authorization, string device, Dictionary<string, object> claimKeys, int expireMinutes = 60)
            {
                var issuer = config.GetValue<string>("JwtSettings:Issuer");
            ...
            public string GenerateEnvryptionToken(string userName, string issuer, List<Claim>? items, int expireMinutes = 60)
            {
                ...
                List<Claim> claims = items ?? new List<Claim>();
                var userClaimsIdentity = new ClaimsIdentity(claims, "Bearer", ClaimsIdentity.DefaultNameClaimType, null, userName);
                // 建立一組對稱式加密的金鑰，主要用於 JWT 簽章之用
                var securityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(signKey));
